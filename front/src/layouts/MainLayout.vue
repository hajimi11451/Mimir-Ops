    ...
    <aside class="w-64 bg-sidebar flex-shrink-0 flex flex-col transition-all duration-300">
      ...
      <nav class="flex-1 py-6 space-y-1">
        ...
        <router-link
          to="/info-list"
          class="group flex items-center px-6 py-3 transition-colors border-l-4"
          :class="$route.path === '/info-list' ? 'bg-dark border-brand text-white' : 'text-gray-400 hover:bg-gray-700 hover:text-white border-transparent'"
        >
          <svg class="mr-3 h-5 w-5" :class="$route.path === '/info-list' ? 'text-brand' : 'group-hover:text-gray-300'" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <span class="text-sm" :class="{ 'font-semibold': $route.path === '/info-list' }">全部信息与告警</span>
        </router-link>
      </nav>
    </aside>
