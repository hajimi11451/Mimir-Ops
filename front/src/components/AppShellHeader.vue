<template>
  <header class="relative z-40 border-b border-white/70 bg-white/88 backdrop-blur-xl">
    ...
    <div class="mx-auto flex h-[68px] w-full max-w-[1600px] items-center justify-between px-4 sm:px-6 lg:px-8">
      ...
      <div class="flex items-center gap-2 sm:gap-3">
        <template v-if="showAuthLinks">
          <span class="hidden rounded-full border border-ui-border bg-ui-panel px-3 py-1 text-xs font-medium text-ui-subtext sm:inline-flex">
            未登录
          </span>
          <router-link
            to="/login"
            class="rounded-full px-4 py-2 text-sm font-medium transition-colors"
            :class="route.path === '/login' ? 'bg-brand text-white shadow-[0_12px_24px_-18px_rgba(37,99,235,1)]' : 'text-ui-subtext hover:bg-ui-panel hover:text-ui-text'"
          >
            登录
          </router-link>
          <router-link
            ...
            :class="route.path === '/register' ? 'border-brand bg-brand/8 text-brand' : 'bg-white text-ui-subtext hover:border-brand/30 hover:text-brand'"
          >
            注册
          </router-link>
        </template>
        ...
        <template v-else>
          <div class="hidden items-center gap-2 rounded-full border border-ui-border bg-ui-panel px-3 py-2 sm:flex">
          ...
          <button
            type="button"
            class="rounded-full border border-ui-border px-4 py-2 text-sm font-medium text-ui-subtext transition-colors hover:border-brand/35 hover:text-brand"
            @click="$emit('logout')"
          >
            退出
          </button>
